#!/bin/bash
#SBATCH --job-name=GWDG_FPIN_AC_N50_M7
#SBATCH --partition=grete:shared
#SBATCH --nodes=1 --ntasks-per-node=1 -G A100:1 --time=48:00:00 --requeue
#SBATCH --mail-type=FAIL --mail-user=thyssens@ismll.de
#SBATCH --output=/projects/extern/nhr/nhr_ni/nim00026/dir.project/daniela/cluster_logs/GWDG_FPIN_AC_N50_M7_%j.log
#SBATCH --error=/projects/extern/nhr/nhr_ni/nim00026/dir.project/daniela/cluster_logs/GWDG_FPIN_AC_N50_M7_%j.err
set -eo pipefail
...
RUN_NAME="fpin_ac_n50_k7_unf_attn"
RESUME_GLOB="${FPIN_OUTPUT_ROOT}/cvrp_50_uniform/train/fpin/${RUN_NAME}_*/checkpoints/best.pt"
LATEST_CKPT=$(ls -t $RESUME_GLOB 2>/dev/null | head -1 || true)
RUN_DIR="${FPIN_OUTPUT_ROOT}/cvrp_50_uniform/train/fpin/${RUN_NAME}_$(date +%Y-%m-%d_%H-%M-%S)"
COMMON=( env=cvrp50_unf model=fpin hydra.job.chdir=true "hydra.run.dir=${RUN_DIR}" \
  ...
  "train_cfg.run_name=${RUN_NAME}" fixed_train_set=/projects/extern/nhr/nhr_ni/nim00026/dir.project/daniela/data/train_data/cvrp/uniform/targets/fc_hgs_clean/n50_k7/ )
