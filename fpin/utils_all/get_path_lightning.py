...
def make_valid(all_idxs,probs,remaining_capa,demand):
    ...
    for j in batch_missing_:
        ...
        if list(available_vs):
            ...
            #print('NO greedy SOL for this instance; no Capacity left for missing customer')
            #all_idxs,final_routes,final_loads,missing_final=None,None,None,None
            
            
    demands_=demands_cut.unsqueeze(2).expand(m,n,n)
    #pred_demands=(demands_*all_idxs)
    ...
    #missing_final=torch.tensor(np.setdiff1d(list(np.arange(1,n)),sorted(all_visited))).long().cuda()
    #print(missing)
    
    if list(missing_final):
        missing_final=1
    else:
        missing_final=0
...
def greedy_path(probs,demand,current_perm):
    ...
    #all_visited=[]
    #for i in range(m):
    #    print(targ_as_lst(all_idxs[i].max(1)[1]),loads[i].item())
    #    all_visited.extend(targ_as_lst(all_idxs[i].max(1)[1],n)[1:-1])
    #missing=torch.tensor(np.setdiff1d(list(np.arange(1,n)),sorted(all_visited))).long().cuda()
    #print(missing)
    #if list(missing):
    #    missing=1
    #else:
...
def load_estimate(probs,demand,perm_m):
    ...
    arr=torch.arange(all_idxs.size(0), device=probs_.device)
    ...
    demands_=demand.unsqueeze(2).expand(b,m,n,n)
    pred_demands=(demands_*all_idxs)
    loads=(demands_*all_idxs).sum(dim=3).sum(dim=2)
    
    return all_idxs,loads,pred_demands
