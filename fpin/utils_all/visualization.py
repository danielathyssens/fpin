...
def _test(problem: str = "cvrp",
    ...
    SAVE_PATH = os.path.join(os.getcwd(), 'visualisations')
    SMP = 32
    N = graph_size
    BS = 16
    BS_ = BS
    ...
    # init data class
    ds = data_class(distribution=distribution, num_samples=1, graph_size=N)

    for sample in ds:
        print(f'sample {sample}')
        # init Viewer Object
        viewer = Viewer(locs=sample.coords, save_dir=SAVE_PATH, gif_naming="test_gif")

        # viewer.plot_locs(locs=sample.coords)
