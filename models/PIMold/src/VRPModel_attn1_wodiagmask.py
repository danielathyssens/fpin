...
class VRP_Net(nn.Module):
    ...
    def __init__(self, layers, depot_in_dim, cities_in_dim, fleet_in_dim, main_dim,
                 ...
                 with_loads, bi_directional, attn_dotproduct, memory_efficient):
        super().__init__()
    ...
    def forward(self, depot, customers, fleet, demands, dists):
        ...
        # Float(batch x cities_length +1 x main_dim):
        cities = torch.cat((depot, customers), dim=1)
        ...
        if self.attn_dotproduct:
            ...
            base_nodes = torch.cat((cities.unsqueeze(2).expand(b, n, n, main_dim),
                                    cities.unsqueeze(1).expand(b, n, n, main_dim)), dim=3)
            ...
            for i, vehicle in enumerate(chunk_at(fleet, dim=1, squeeze=False)):  # Float(batch x 1 x main_dim)
                ...
                vehicle_exp = vehicle.expand(b, n * n, vehicle.size(2))  # Float(batch x (n+1*n+1) x main_dim)

                nodes = torch.cat((vehicle_exp, base_nodes), dim=2)
                nodes = self.linear_out_1(nodes)  # Float(batch x (n+1*n+1) x main_dim)
                nodes = F.relu(nodes)  # Float(batch x (n+1*n+1) x main_dim)

                ##### Replace linear_out_2 with PART-ATTN #####
                logits = torch.matmul(vehicle, nodes.transpose(1, 2))  # Float(batch x 1 x (n+1*n+1))
                # print('nodes.size(1)',nodes.size(1))
                logits = logits.squeeze(-2) / math.sqrt(nodes.size(1))  # Float(batch x (n+1*n+1))
                scores = torch.softmax(logits / 1.0, dim=-1)  # Float(batch x (n+1*n+1))
                scores = scores.unsqueeze(-1)
                scores = scores.view(b, n, n)  # Float(batch x n+1 x n+1)
                res.append(scores)
            ...
            if self.memory_efficient:
                from_cities = cities.unsqueeze(2).expand(b, n, n, main_dim)  # Float(batch x n+1 x main_dim)
                to_cities = cities.unsqueeze(1).expand_as(from_cities)
                base_nodes = torch.cat((from_cities, to_cities), dim=3)  # Float(batch x (n+1xn+1) x (main_dim*2))
                base_nodes = base_nodes.view(base_nodes.size(0),
                                             n * n, main_dim * 2)  # Float(batch x (n+1xn+1) x (main_dim*2))

                res = []
                for i, vehicle in enumerate(chunk_at(fleet, dim=1, squeeze=False)):  # Float(batch x 1 x main_dim)
                    vehicle = vehicle.expand(b, n * n, vehicle.size(2))  # Float(batch x (n+1xn+1) x main_dim)
                    nodes = torch.cat((vehicle, base_nodes), dim=2)  # Float(batch x (n+1xn+1) x (main_dim*3))
                    nodes = self.linear_out_1(nodes)
                    nodes = F.relu(nodes)
                    nodes = self.linear_out_2(nodes)  # Float(batch x (n+1xn+1) x 1)
                    nodes = nodes.view(b, n, n)  # Float(batch x n+1 x n+1)
                    res.append(nodes)
                # SCORES
                output = torch.stack(res, dim=1)  # Float(batch x m x n+1 x n+1)
        ...
        # result = self.softassign(output,demands)
        # result=output
        ...
        if self.with_loads:
            ...
            # loads (bi-directional):          Float(batch x 2 x fleet)
            # pred_demands (uni-directional):  Float(batch x fleet x n x n)
            # pred_demands (bi-directional):   Float(batch x 2 x fleet x n x n)

            # get random permutation of vehicles
            perm_m = list(range(m))
            random.shuffle(perm_m)

            # BI-DIRECTIONAL PATHS and LOADS
            if self.bi_directional:
                path_idx_to, loads_to, pred_demands_to = load_estimate(result, demands, perm_m)
                path_idx_from, loads_from, pred_demands_from = load_estimate(result.transpose(2, 3), demands, perm_m)
                pred_demands = torch.stack((pred_demands_from, pred_demands_to), dim=1)
                pred_loads = torch.stack((loads_from, loads_to), dim=1)
                sample_path_b0 = [targ_as_lst(path_idx_to[0][i].max(1)[1]) for i in range(m)]
                path_idx = path_idx_to
            # ONLY TO-DIRECTION PATHS and LOADS
            else:
                path_idx, pred_loads, pred_demands = load_estimate(output, demands, perm_m)
        ...
        # print("\tIn Model: input (customers) size", customers.size(), "output (results) size", result.size())

        return output, path_idx, pred_loads, pred_demands, sample_path_b0
