...
class VRPLoss(nn.Module):
    def __init__(self, start_weight=0.5, simple_loss=False,
                 no_perms=False, size_average=True, with_penalty=True, with_load_loss=True):
    ...
    def forward(self, probs, loads, target, targ_loads):
        ...
        # calc permutation invariant loss
            ...
            # initial targ_dems:     Float(batch x fleet x from_city x to_city)
            # initial logProbs:      Float(batch x fleet x from_city x to_city)
            # initial demands_pred:  Float(batch x fleet x from_city x to_city) or Float(batch x 2 x fleet x from_city x to_city)
            # initial loads:         Float(batch x fleet) or Float(batch x 2 x fleet)
            # -----------------------------------------------------------------
            # transf. target:        Float(batch x fleet x 2 x fleet x from_city x to_city)
            # transf. targ_dems:     Float(batch x fleet x 2 x fleet x from_city x to_city)
            # transf. logProbs:      Float(batch x fleet x 2 x fleet x from_city x to_city)
            # transf. demands_pred:  Float(batch x fleet x 2 x fleet x from_city x to_city)
            # transf. loads:         Float(batch x fleet x 2 x fleet)
            ...
            if m >= 10:
                ...
                for perms in torch.tensor_split(perms_all, 115):
                    ...
                    if self.with_load_loss:
                        if self.with_load_loss:
                            ...
                            weighted_losses_ = weighted_losses.gather(dim=1,
                                                                      index=perms)  # Float(batch x perms x groups)
                            # NOW BOTH weighted_losses and perm_load_losses are [b,perms,m]
                            weighted_losses_l = weighted_losses_ + 0.5 * perm_load_losses  # Float(batch x perms x groups)
                ...
                if self.with_load_loss:
                    ...
                    loads_perms = loads_.gather(dim=1, index=perms) # Float(batch x perms x groups)
                    # targ_loads = targ_loads.unsqueeze(1).unsqueeze(2).expand_as(loads_perms)
                    ...
                    # weighted_losses_f = weighted_losses + 0.5*perm_load_losses
                else:
                    weighted_losses = weighted_losses.gather(dim=1, index=perms)  # Float(batch x perms x groups)
