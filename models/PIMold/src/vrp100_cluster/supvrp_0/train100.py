...
def train(opts):
    ...
    # parameters
    model_params = {'with_loads': opts.load, 'attn_dotproduct': True, 'memory_efficient': True, 'bi_directional': False,
    ...
    loss_params = {'simple_loss': True, 'no_perms': False, 'size_average': True, 'with_penalty': opts.pen_l,
                   'with_direct_demand': opts.dem_l, 'with_loads_loss': opts.load_l}
    ...
    class HParams(object):
        def __init__(self, lr=None, wd=None, dropout=None, batch_s=None, n_hidden=None, layers=None,
            ...
            self.starts_weight = 0.15 if starts_weight is None else starts_weight
    ...
    class Dataset_VRP(torch.utils.data.Dataset):
        ...
        def __init__(self, list_IDs, X_dat, Y_idx, Y_val, Y_size, YLoad_dat):
            """Initialization"""
        ...
        def __getitem__(self, index):
            ...
            y_idx = self.Y_idx[ID]
            y_val = self.Y_val[ID]
            y_size = self.Y_size[ID]
            y_load = self.YLoad_dat[ID]

            return X, y_idx, y_val, y_size, y_load

    def sparse_batch_collate(batch: list):

        # X, y_i, y_v, y_s, y_load = zip(*batch)
        fleet_batch = torch.stack([torch.tensor(item[0][0]) for item in batch])
        # print(fleet_batch.size())
        depot_batch = torch.stack([torch.tensor(item[0][1]) for item in batch])
        # print(depot_batch.size())
        custom_batch = torch.stack([torch.tensor(item[0][2]) for item in batch])
        # print(custom_batch.size())
        demand_batch = torch.stack([torch.tensor(item[0][3]) for item in batch])
        # print(demand_batch.size())
        dists_batch = torch.stack([torch.tensor(item[0][4]) for item in batch])
        # print(dists_batch.size())
        x_batch = [fleet_batch, depot_batch, custom_batch, demand_batch, dists_batch]
        yl_batch = torch.stack([item[4] for item in batch]).float()

        y_i = [item[1] for item in batch]
        y_v = [item[2] for item in batch]
        y_s = [(int(item[3][0]), int(item[3][1]), int(item[3][2])) for item in batch]

        return [x_batch, y_i, y_v, y_s, yl_batch]

    # import training and validation data
    data_X, Y_i, Y_v, Y_s, data_YLoad = import_trainval_data(vrp_to_solve, nr_files)

    # shuffle training instances
    c = list(zip(data_X, Y_i, Y_v, Y_s, data_YLoad))
    random.shuffle(c)
    data_X, Y_i, Y_v, Y_s, data_YLoad = zip(*c)
    tr = len(data_X) - 10000
    ...
    # Generators
    training_set = Dataset_VRP(partition['train'], data_X, Y_i, Y_v, Y_s, data_YLoad)
    print('training_set.__len__()', training_set.__len__())
    val_set = Dataset_VRP(partition['val'], data_X, Y_i, Y_v, Y_s, data_YLoad)
    training_generator = DataLoader(training_set, collate_fn=sparse_batch_collate, **dat_params)
    val_generator = DataLoader(val_set, collate_fn=sparse_batch_collate, **dat_params_v)
    ...
    max_epochs = 100
    # some log lists
    training_loss_eps, validation_loss_eps, CapViolation_eps, CapViolation_mean_eps = [], [], [], []
    ...
    for epoch in range(max_epochs):
        ...
        model.train()
        # batch-wise logs
        train_loss_batches, violations_batch, violations_batch_mean = [], [], []
        for local_batch, y_i, y_v, y_s, target_loads in training_generator:
            ...
            targets = torch.stack([torch.sparse_coo_tensor(i, v, s) for i, v, s in zip(y_i, y_v, y_s)])
            ...
            # Zero gradients
            optimizer.zero_grad()
            vrp_probs, vrp_loads, sample_path_b0 = model(depot_batch, custom_batch, fleet_batch,
                                                         demand_batch, dists_batch)
            if vrp_loads is not None:
                capa_violation_sum = torch.where(vrp_loads > 1.00001, vrp_loads - 1.0000, zeros(1)).sum(
                    1)  # Float(batch)
        ...
        with torch.set_grad_enabled(False):
            for local_batch, y_i_v, y_v_v, y_size_v, target_loads_v in val_generator:
                # unpack X_groups
                ...
                targets_v = torch.stack([torch.sparse_coo_tensor(i, v, s) for i, v, s in zip(y_i_v, y_v_v, y_size_v)])
                ...
                vrp_probs_v, vrp_loads_v, sample_path_b0_v = model(depot_batch_v, custom_batch_v,
                                                                   fleet_batch_v,
                                                                   demand_batch_v, dists_batch_v)
        ...
        if epoch % 2 == 0:
            print('\nEpoch: {}, TRAIN loss (avg of all batches): {}'.format(epochs_done, np.mean(train_loss_batches)))
            # print_func(VRP_probs,sample_path_b0,VRP_loads,targets,target_loads,model_params['with_loads'],VRP50,v5)
            print('\nEpoch: {}, VALID loss (avg of all batches): {}'.format(epochs_done, np.mean(valid_loss_batches)))

            if vrp_loads is not None:
                print('Epoch: {}, CAPA VIOL (avg of all batches): {}'.format(epochs_done, np.mean(violations_batch)))
    ...
    return epochs_done, best_loss, best_model, validation_loss_eps, training_loss_eps, CapViolation_eps, CapViolation_mean_eps


if __name__ == "__main__":
    num_eps, best_val_loss, model_params, val_loss, train_loss, CapViolation, CapViolation_mean = train(get_options())
    ...
    torch.save(model_params, os.path.join(save_outs, 'VRP_model_100.pth'))
