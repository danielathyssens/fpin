...
def transform_Xs(X_dat, X_dat_kool, nr_of_data=95000):
...
def transform_targets(Y_dat, m=7, n=51, capa=40):
    ...
    for Y in Y_dat:
        ...
        target_acc = torch.from_numpy(target_accD)
        all_target_loads.append(target_acc)
        count += 1

    return all_target_loads
...
def import_trainval_data(vrp_solved, nr_files):
    ...
    # transform Ys
    dat_YLoad = transform_targets(Y_cleaned, m, n, capa)

    y_i = torch.load('data/' + vrp_solved + '_new/targets_i.pt')
    y_v = torch.load('data/' + vrp_solved + '_new/targets_v.pt')
    y_s = torch.load('data/' + vrp_solved + '_new/targets_s.pt')
    y_i = y_i[:len(dat_YLoad)]
    y_v = y_v[:len(dat_YLoad)]
    y_s = y_s[:len(dat_YLoad)]
    
    print('len(y_i)',len(y_i))
    print('len(y_v)',len(y_v))
    print('len(y_s)',len(y_s))
    print('len(dat_X)',len(dat_X))
    print('len(dat_YLoad)',len(dat_YLoad))
    
    #y_dense = torch.load('data/' + vrp_solved + '_new/targets_dense.pt')
    
    #for g in range(0,3):
    #    y_sp = torch.sparse_coo_tensor(y_i[g], y_v[g], (int(y_s[g][0]), int(y_s[g][1]), int(y_s[g][2]))).to_dense()
    #    y_de = y_dense[g]
    #    print(y_sp[0][0])
    #    print(y_de[0][0])
    #    print(y_sp.size())
    #    print(y_de.size())
    #    print('(y_sp == y_de).all()',(y_sp == y_de).all())
    #, y_dense
    return dat_X, y_i, y_v, y_s, dat_YLoad
