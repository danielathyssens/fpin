...
class DistancesCapaToWeights(nn.Module):
    ...
    def forward(self, dists, demands):
        ...
        # weights_v = fleet_
        # demands # Float(b x m x n x main_dim)
        ...
        # weights_v = weights_v * self.gamma_v.unsqueeze(0).unsqueeze(1).unsqueeze(2) + \
        #    self.beta_v.unsqueeze(0).unsqueeze(1).unsqueeze(2)
    ...
    def forward(self, depot, customers, fleet, demands, dists):
        ...
        if self.attn_dotproduct:
            ...
            for i, vehicle in enumerate(chunk_at(fleet, dim=1, squeeze=False)):  # Float(batch x 1 x main_dim)
                ...
                logits = logits.squeeze(-2) / math.sqrt(
                    vehicle.size(-1))  # Float(batch x (cities_length*cities_length))
        ...
        mask_diag[0, 0] = 0
        result = output.masked_fill(mask_diag.unsqueeze(0).unsqueeze(1).expand(b, m, n, n), min_inf)
